...
BEGIN   
  ...
  THEN				
                ...
                INSERT INTO SYS.AUDIT_INFO_TRACE (logon_time, sid, serial, username, machine, tracefile) select CURRENT_TIMESTAMP,se.sid,se.serial#,se.username,se.machine,tracefile from sys.v$process pro, sys.gv$session se WHERE se.sid = Sys_Context('USERENV', 'SID') and AUDSID = Sys_Context('USERENV', 'SESSIONID') AND pro.addr = se.paddr;
  END IF;
EXCEPTION
WHEN OTHERS THEN
NULL;
